...
/**
 * Generate a specific, report-tailored Executive PDF for Admin / History downloads
 */
export async function generateSpecificExecutiveReport(
  report: ExecutiveReportItem,
): Promise<void> {
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
  const companyName = report.comp || "Enterprise Company";
  const TOTAL = 2;
  const TOP = 27;

  // Sanitized Filename e.g. "SWOT_Strategic_Intelligence_Deck.pdf"
  ...
  if (lowerName.includes("swot")) {
    // ---- 1. SWOT STRATEGIC INTELLIGENCE DECK ----
    drawHeader(doc, companyName);
    drawFooter(doc, 1, TOTAL);
    let y = TOP;
    ...
    doc.text("SWOT Strategic Intelligence Deck", MARGIN + 8, y + 10);
    doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(...C.slate400);
    doc.text("Autonomous C-Suite Strategic Assessment & Threat Intelligence Audit", MARGIN + 8, y + 16.5);

    const meta = [
      { label: "Company", val: companyName },
      { label: "Generated By", val: report.by },
      { label: "Report Date", val: report.date },
      { label: "Engine", val: "EvoAI Core v4.0" },
    ];
    meta.forEach((m, i) => {
      const mx = MARGIN + 8 + i * (CONTENT_W / 4);
    ...
    // Section 1: Executive SWOT Index & Summary
    y = sectionHeader(doc, "1. Executive SWOT Index & Intelligence Summary", y);

    const cW = (CONTENT_W - 9) / 4;
    kpiCard(doc, MARGIN, y, cW, 20, "AI Health Index", "94 / 100", "Top 5% Benchmark", C.indigo);
    kpiCard(doc, MARGIN + cW + 3, y, cW, 20, "Market Defense", "88 / 100", "Strong Position", C.emerald);
    kpiCard(doc, MARGIN + 2 * (cW + 3), y, cW, 20, "Growth Momentum", "+38.3% YoY", "High Acceleration", C.cyan);
    kpiCard(doc, MARGIN + 3 * (cW + 3), y, cW, 20, "Threat Vulnerability", "Low-Medium", "Monitored", C.amber);
    y += 24;

    // Section 2: Comprehensive SWOT Matrix Table
    y = sectionHeader(doc, "2. Comprehensive SWOT Matrix Table", y);

    autoTable(doc, {
      startY: y,
      margin: { left: MARGIN, right: MARGIN },
      head: [["Category", "Strategic Vector", "Impact Rating", "Operational Insight"]],
      body: [
        ["Strengths", "Industry-leading AI Accuracy (94%)", "Critical Advantage", "Proprietary model fine-tuning yields superior precision"],
        ["Strengths", "High Enterprise Retention (118% NRR)", "High Advantage", "Strong product-market fit across tier-1 accounts"],
        ["Weaknesses", "Brand Awareness in APAC Region", "Moderate Vulnerability", "Lower sales headcount compared to regional legacy leaders"],
        ["Weaknesses", "Single Core Product Dependency", "Moderate Vulnerability", "Requires expansion into modular add-on microservices"],
        ["Opportunities", "APAC Regional Market Expansion", "$4.2M Opportunity", "High unserved enterprise demand in Southeast Asia"],
        ["Opportunities", "Regulatory AI Compliance Demand", "$2.8M Opportunity", "EU AI Act driving urgency for audited AI platforms"],
        ["Threats", "Nexus AI Aggressive APAC Push", "High Exposure", "Competitor offering subsidized entry pricing"],
        ["Threats", "Macroeconomic Tech Budget Cutbacks", "Medium Exposure", "Longer enterprise procurement cycle times"],
      ],
      styles: { fontSize: 7.5, cellPadding: 2.5, textColor: [51, 65, 85] },
      headStyles: { fillColor: C.slate900, textColor: C.white, fontStyle: "bold", fontSize: 7.5 },
      alternateRowStyles: { fillColor: C.slate50 },
      ...
      didParseCell: (d: any) => {
        if (d.section === "body" && d.column.index === 0) {
          if (d.cell.text[0] === "Strengths") d.cell.styles.textColor = C.emerald;
          else if (d.cell.text[0] === "Weaknesses") d.cell.styles.textColor = C.amber;
          else if (d.cell.text[0] === "Opportunities") d.cell.styles.textColor = C.indigo;
          else if (d.cell.text[0] === "Threats") d.cell.styles.textColor = C.rose;
        }
      },
    });
    ...
    // Section 3: Risk Vector Heatmap & Action Plan
    y = sectionHeader(doc, "3. Risk Vector Heatmap & Strategic Recommendations", y);

    autoTable(doc, {
      startY: y,
      margin: { left: MARGIN, right: MARGIN },
      head: [["#", "Actionable Recommendation", "Owner Agent", "Timeline", "Priority"]],
      body: [
        ["1", "Accelerate APAC expansion before Nexus AI consolidates regional pricing", "CEO Agent", "Week 1-4", "Critical"],
        ["2", "Deploy automated EU AI Act compliance audit trails for enterprise clients", "Legal & Audit Agent", "Week 2-6", "High"],
        ["3", "Expand modular microservices pricing to diversify single-product dependency", "Finance Agent", "Week 3-8", "High"],
        ["4", "Launch targeted APAC brand awareness campaign on enterprise channels", "Marketing Agent", "Week 4-12", "Medium"],
      ],
      styles: { fontSize: 7.5, cellPadding: 2.5 },
      headStyles: { fillColor: C.slate900, textColor: C.white, fontStyle: "bold", fontSize: 7.5 },
      alternateRowStyles: { fillColor: C.slate50 },
      ...
      didParseCell: (d: any) => {
        if (d.section === "body" && d.column.index === 4) {
          if (d.cell.text[0] === "Critical") d.cell.styles.textColor = C.rose;
          else if (d.cell.text[0] === "High") d.cell.styles.textColor = C.amber;
        }
      },
    });
    ...
    doc.text("EXECUTIVE AUDIT & GOVERNANCE SIGN-OFF", MARGIN + 4, y + 5.5);
    doc.setFontSize(6.5); doc.setFont("helvetica", "normal"); doc.setTextColor(...C.slate600);
    doc.text(`SWOT Strategic Intelligence Report compiled for ${companyName} (${report.by}).`, MARGIN + 4, y + 10.5);
    doc.text(`Audit Security Hash: SWOT-2026-${Math.floor(100000 + Math.random() * 900000)} | Classification: STRICTLY CONFIDENTIAL`, MARGIN + 4, y + 15.5);
  ...
  if (lowerName.includes("revenue") || lowerName.includes("forecast") || lowerName.includes("unit economics")) {
    // ---- 2. Q4 REVENUE FORECAST & UNIT ECONOMICS ----
    drawHeader(doc, companyName);
    drawFooter(doc, 1, TOTAL);
    let y = TOP;
    ...
    doc.setFontSize(14); doc.setFont("helvetica", "bold"); doc.setTextColor(...C.white);
    doc.text("Q4 Revenue Forecast & Unit Economics", MARGIN + 8, y + 10);
    doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(...C.slate400);
    doc.text("Financial Telemetry, Forecast Models & Gross Margin Analysis", MARGIN + 8, y + 16.5);
    ...
    // Section 1: Financial Telemetry & Key Unit Metrics
    y = sectionHeader(doc, "1. Financial Telemetry & Key Unit Metrics", y);

    const cW = (CONTENT_W - 9) / 4;
    kpiCard(doc, MARGIN, y, cW, 20, "Annual Run Rate", "$24.72M", "+38.3% YoY", C.indigo);
    kpiCard(doc, MARGIN + cW + 3, y, cW, 20, "Monthly Rec. Rev", "$2.06M", "+3.2% MoM", C.purple);
    kpiCard(doc, MARGIN + 2 * (cW + 3), y, cW, 20, "Net Profit Margin", "31.4%", "+5.1% Optimal", C.emerald);
    kpiCard(doc, MARGIN + 3 * (cW + 3), y, cW, 20, "LTV / CAC Ratio", "4.8x", "Healthy Benchmark", C.cyan);
    y += 24;

    // Section 2: Q4 Revenue Forecast Schedule Table
    y = sectionHeader(doc, "2. Q4 Revenue & Profit Forecast Schedule", y);

    autoTable(doc, {
      startY: y,
      margin: { left: MARGIN, right: MARGIN },
      head: [["Period", "Projected Revenue", "Baseline Target", "Projected OpEx", "Net Profit", "Margin %"]],
      body: [
        ["Jul '26", "$3,100,000", "$2,900,000", "$2,100,000", "$1,000,000", "32.2%"],
        ["Aug '26", "$3,450,000", "$3,000,000", "$2,300,000", "$1,150,000", "33.3%"],
        ["Sep '26", "$3,900,000", "$3,150,000", "$2,500,000", "$1,400,000", "35.8%"],
        ["Oct '26", "$4,300,000", "$3,300,000", "$2,700,000", "$1,600,000", "37.2%"],
        ["Nov '26", "$4,800,000", "$3,450,000", "$2,950,000", "$1,850,000", "38.5%"],
        ["Dec '26", "$5,400,000", "$3,600,000", "$3,200,000", "$2,200,000", "40.7%"],
      ],
      styles: { fontSize: 7.5, cellPadding: 2.5, textColor: [51, 65, 85] },
      headStyles: { fillColor: C.slate900, textColor: C.white, fontStyle: "bold", fontSize: 7.5 },
      alternateRowStyles: { fillColor: C.slate50 },
      columnStyles: {
        0: { fontStyle: "bold" },
        1: { textColor: C.indigo, fontStyle: "bold" },
        4: { textColor: C.emerald, fontStyle: "bold" },
      },
    });
    y = ((doc as any).lastAutoTable?.finalY ?? y + 40) + 6;
    ...
    // Section 3: Expense Allocation & Unit Economics
    y = sectionHeader(doc, "3. Expense Breakdown & Unit Economics Optimization", y);

    autoTable(doc, {
      startY: y,
      margin: { left: MARGIN, right: MARGIN },
      head: [["Expense Category", "% Share", "Monthly Spend", "Efficiency Benchmark", "Action Status"]],
      body: [
        ["Engineering & Personnel", "42%", "$1,193,000", "Optimal Team Size", "Maintained"],
        ["Cloud Infrastructure", "18%", "$511,000", "Auto-scaling Target", "Optimizing"],
        ["Sales & Marketing", "15%", "$426,000", "High Conversion Yield", "Expanding"],
        ["Operations & Facilities", "13%", "$369,000", "Fixed Overhead", "On Track"],
        ["R&D & Software Licenses", "12%", "$341,000", "Innovation Budget", "On Track"],
      ],
      styles: { fontSize: 7.5, cellPadding: 2.5 },
      headStyles: { fillColor: C.slate900, textColor: C.white, fontStyle: "bold", fontSize: 7.5 },
      alternateRowStyles: { fillColor: C.slate50 },
    ...
    doc.text("EXECUTIVE AUDIT & GOVERNANCE SIGN-OFF", MARGIN + 4, y + 5.5);
    doc.setFontSize(6.5); doc.setFont("helvetica", "normal"); doc.setTextColor(...C.slate600);
    doc.text(`Financial Forecast Report verified for ${companyName} (${report.by}).`, MARGIN + 4, y + 10.5);
    doc.text(`Audit Security Hash: FIN-2026-${Math.floor(100000 + Math.random() * 900000)} | Classification: STRICTLY CONFIDENTIAL`, MARGIN + 4, y + 15.5);
  ...
  if (lowerName.includes("competitor") || lowerName.includes("telemetry") || lowerName.includes("intelligence")) {
    // ---- 3. COMPETITOR INTELLIGENCE TELEMETRY ----
    drawHeader(doc, companyName);
    drawFooter(doc, 1, TOTAL);
    let y = TOP;
    ...
    doc.setFontSize(14); doc.setFont("helvetica", "bold"); doc.setTextColor(...C.white);
    doc.text("Competitor Intelligence Telemetry", MARGIN + 8, y + 10);
    doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(...C.slate400);
    doc.text("Real-Time Competitor Benchmarking, Pricing & Market Share Telemetry", MARGIN + 8, y + 16.5);
    ...
    // Section 1: Competitive Telemetry Overview
    y = sectionHeader(doc, "1. Competitive Landscape Overview", y);

    const cW = (CONTENT_W - 9) / 4;
    kpiCard(doc, MARGIN, y, cW, 20, "Your Share", "16.4%", "+3.2% YoY", C.purple);
    kpiCard(doc, MARGIN + cW + 3, y, cW, 20, "Market Leader", "31.0% (TCS)", "Challenger Position", C.indigo);
    kpiCard(doc, MARGIN + 2 * (cW + 3), y, cW, 20, "Price Metric", "$360 / mo", "High Premium Yield", C.emerald);
    kpiCard(doc, MARGIN + 3 * (cW + 3), y, cW, 20, "Threat Defense", "92 / 100", "Strong Moat", C.cyan);
    y += 24;

    // Section 2: Competitor Benchmark Matrix
    y = sectionHeader(doc, "2. Competitor Benchmark & Pricing Matrix", y);

    autoTable(doc, {
      startY: y,
      margin: { left: MARGIN, right: MARGIN },
      head: [["Company", "Market Share", "Growth YoY", "Pricing Tier", "Threat Category", "Recent Telemetry Activity"]],
      body: [
        [`${companyName} (You)`, "16.4%", "+24.8%", "$360 / mo", "Your Business", "Expanded enterprise AI automation suite"],
        ["TCS", "31.0%", "+12.0%", "$1,200 / mo", "Market Leader", "Won $500M digital cloud transformation contract"],
        ["Infosys", "25.0%", "+14.0%", "$1,100 / mo", "Challenger", "Launched generative AI platform for enterprise"],
        ["Wipro", "18.0%", "+8.0%", "$950 / mo", "Global IT", "Expanded European consulting & delivery practice"],
        ["Accenture", "14.0%", "+16.0%", "$1,400 / mo", "Consulting Giant", "Acquired 3 regional AI & cybersecurity boutiques"],
      ],
      styles: { fontSize: 7.5, cellPadding: 2.5, textColor: [51, 65, 85] },
      headStyles: { fillColor: C.slate900, textColor: C.white, fontStyle: "bold", fontSize: 7.5 },
      alternateRowStyles: { fillColor: C.slate50 },
      ...
      didParseCell: (d: any) => {
        if (d.section === "body" && d.row.index === 0) {
          d.cell.styles.fillColor = [240, 245, 255];
          d.cell.styles.textColor = C.indigo;
        }
      },
    });
    y = ((doc as any).lastAutoTable?.finalY ?? y + 40) + 6;
    ...
    // Section 3: Strategic Counter-Actions & Recommendations
    y = sectionHeader(doc, "3. Competitive Positioning & Strategic Counter-Actions", y);

    autoTable(doc, {
      startY: y,
      margin: { left: MARGIN, right: MARGIN },
      head: [["#", "Strategic Action Item", "Assigned Agent", "Target Competitor", "Impact"]],
      body: [
        ["1", "Highlight superior retention (118% NRR) against TCS high enterprise pricing", "Radar Agent", "TCS", "High Yield"],
        ["2", "Protect core accounts against Infosys generative AI marketing push", "Growth Agent", "Infosys", "Defensive"],
        ["3", "Capture mid-market accounts leaving Accenture due to fee inflation", "Sales Agent", "Accenture", "Growth Expansion"],
      ],
      styles: { fontSize: 7.5, cellPadding: 2.5 },
      headStyles: { fillColor: C.slate900, textColor: C.white, fontStyle: "bold", fontSize: 7.5 },
      alternateRowStyles: { fillColor: C.slate50 },
      columnStyles: { 0: { halign: "center", fontStyle: "bold" }, 4: { fontStyle: "bold", textColor: C.emerald } },
    });
    y = ((doc as any).lastAutoTable?.finalY ?? y + 30) + 10;
    ...
    doc.text("EXECUTIVE AUDIT & GOVERNANCE SIGN-OFF", MARGIN + 4, y + 5.5);
    doc.setFontSize(6.5); doc.setFont("helvetica", "normal"); doc.setTextColor(...C.slate600);
    doc.text(`Competitor Telemetry Report compiled for ${companyName} (${report.by}).`, MARGIN + 4, y + 10.5);
    doc.text(`Audit Security Hash: COMP-2026-${Math.floor(100000 + Math.random() * 900000)} | Classification: STRICTLY CONFIDENTIAL`, MARGIN + 4, y + 15.5);
  ...
  // ---- 4. GENERIC CUSTOM REPORT FALLBACK ----
  drawHeader(doc, companyName);
  drawFooter(doc, 1, 1);
  let y = TOP;
  ...
  doc.setFontSize(14); doc.setFont("helvetica", "bold"); doc.setTextColor(...C.white);
  doc.text(report.name, MARGIN + 8, y + 10);
  doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(...C.slate400);
  doc.text("Autonomous C-Suite Intelligence Executive Audit Document", MARGIN + 8, y + 16.5);
  ...
  y = sectionHeader(doc, `Executive Telemetry - ${report.name}`, y);

  autoTable(doc, {
    startY: y,
    margin: { left: MARGIN, right: MARGIN },
    head: [["Metric Parameter", "Current Telemetry Value", "Target Metric", "Status"]],
    body: [
      ["Executive Health Index", "92 / 100", "95 / 100", "Optimal"],
      ["Operating Capital Yield", "31.4% Margin", "30.0% Target", "Exceeded"],
      ["Risk Defense Score", "88 / 100", "85 / 100", "Passed"],
      ["AI Audit Velocity", "100% Verification", "100%", "Verified"],
    ],
    styles: { fontSize: 7.5, cellPadding: 2.5 },
    headStyles: { fillColor: C.slate900, textColor: C.white, fontStyle: "bold", fontSize: 7.5 },
    alternateRowStyles: { fillColor: C.slate50 },
    columnStyles: { 0: { fontStyle: "bold" }, 3: { textColor: C.emerald, fontStyle: "bold" } },
  });
  y = ((doc as any).lastAutoTable?.finalY ?? y + 30) + 10;
  ...
  doc.text("EXECUTIVE AUDIT & GOVERNANCE SIGN-OFF", MARGIN + 4, y + 5.5);
  doc.setFontSize(6.5); doc.setFont("helvetica", "normal"); doc.setTextColor(...C.slate600);
  doc.text(`Executive Report compiled for ${companyName} (${report.by}).`, MARGIN + 4, y + 10.5);
  doc.text(`Audit Security Hash: REP-2026-${Math.floor(100000 + Math.random() * 900000)} | Classification: STRICTLY CONFIDENTIAL`, MARGIN + 4, y + 15.5);
