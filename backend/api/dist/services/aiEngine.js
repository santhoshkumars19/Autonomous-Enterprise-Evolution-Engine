...
exports.generatePersonalizedAIAnalysis = generatePersonalizedAIAnalysis;
const db_1 = require("../config/db");
async function generatePersonalizedAIAnalysis(input) {
    const { userId, companyId, companyName, revenue, expenses, inventoryValue = Math.round(revenue * 0.15), activeCustomers = Math.max(10, Math.round(revenue / 5000)), churnRate = 3.5, growthRate = 18.5, currency = "USD", } = input;
    ...
    const formatCurrency = (val) => {
        if (currency === "INR" || val >= 100000) {
            if (val >= 10000000)
                return `${currency === "INR" ? "₹" : "$"}${(val / 10000000).toFixed(2)}Cr`;
            if (val >= 100000)
                return `${currency === "INR" ? "₹" : "$"}${(val / 100000).toFixed(2)}L`;
        }
        if (val >= 1000000)
            return `${currency === "INR" ? "₹" : "$"}${(val / 1000000).toFixed(2)}M`;
        if (val >= 1000)
            return `${currency === "INR" ? "₹" : "$"}${(val / 1000).toFixed(0)}K`;
    ...
    // 2. Executive Summary
    const executiveSummary = `Autonomous AI Analysis for ${companyName}: The company recorded total revenue of ` +
        ...
        `Primary AI growth priority: ${profitMargin < 15
    ...
    const existing = await (0, db_1.query)("SELECT id FROM ai_analysis WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1", [userId]);
    let analysisRecord;
    if (existing.length > 0) {
        [analysisRecord] = await (0, db_1.query)(`UPDATE ai_analysis
       ...
       RETURNING *`, [
        ...
        [analysisRecord] = await (0, db_1.query)(`INSERT INTO ai_analysis
