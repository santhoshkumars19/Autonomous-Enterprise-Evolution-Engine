...
async function getUserBusinessMetrics(userId) {
    const userRows = await (0, db_1.query)("SELECT name, email, company, company_id FROM users WHERE id = $1", [userId]);
...
router.get("/overview", async (req, res) => {
    try {
        ...
        const indKpis = (0, industryEngine_1.getIndustryFinancialKPIs)(ctx);
        const kpis = {
            revenue: {
...
router.get("/revenue-forecast", async (req, res) => {
    try {
        ...
        const data = analysis.salesTrend.map((st) => ({
            month: st.month,
            actual: st.revenue,
...
router.post("/update-metrics", async (req, res) => {
    try {
        ...
        const [inserted] = await (0, db_1.query)(`INSERT INTO business_metrics
       ...
       RETURNING *`, [
        ...
        const newAnalysis = await (0, aiEngine_1.generatePersonalizedAIAnalysis)({
            userId,
            companyId: user?.company_id || undefined,
