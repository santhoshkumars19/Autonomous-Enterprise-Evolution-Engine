...
router.put("/:id", async (req, res) => {
    try {
        ...
        const [updatedTask] = await (0, db_1.query)(`UPDATE tasks SET ${setClauses}, updated_at = NOW()
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)("DELETE FROM tasks WHERE id = $1 AND user_id = $2 RETURNING id", [taskId, req.user.id]);
        if (result.length === 0) {
            res.status(404).json({ success: false, message: "Task not found" });
            return;
        }
