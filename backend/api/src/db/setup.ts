...
async function setupDatabase() {
  ...
  try {
    ...
    const existingA = await pool.query("SELECT id FROM users WHERE email = $1", [userAEmail]);
    ...
    const existingB = await pool.query("SELECT id FROM users WHERE email = $1", [userBEmail]);
    if (existingB.rowCount === 0) {
      const hashB = await bcrypt.hash("User@123", 12);
      const resB = await pool.query(
        `INSERT INTO users (name, email, password_hash, company, role)
         VALUES ($1, $2, $3, $4, 'user') RETURNING id`,
        ["User B (Regional Logistics)", userBEmail, hashB, "Company B Logistics"]
      );
      const userBId = resB.rows[0].id;
      await pool.query(
        `INSERT INTO business_metrics (user_id, revenue, expenses, active_customers, churn_rate, growth_rate, currency)
         VALUES ($1, 800000, 720000, 42, 6.4, 8.2, 'INR')`,
        [userBId]
      );
